...
class ResearcherAgent(BaseAgent):
    ...
    def run(self, state: ResearchState) -> ResearchState:
        ...
        source_digest = "\n".join(
            f"- {source.title}: {source.snippet} ({source.url or 'no-url'})" for source in state.sources
        )
        response = self.llm_client.complete(
            system_prompt=(
                "You are a research agent. Summarize the most relevant findings, list consensus points, "
